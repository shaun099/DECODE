...
export default function SyntaxHuntGame({ onWin, onExit, nextClue }: Props) {
  ...
  function click(i: number) {
    ...
    if (!bug.lines[i].trim()) return;          // blank lines are not answers
  ...
  if (phase === 'intro') {
    return (
      <Center>
        <h1 className="font-mono text-3xl font-bold tracking-widest text-green-300">SYNTAX HUNT</h1>
        <p className="max-w-md font-mono text-sm leading-relaxed text-zinc-300">
          Six Python programs. Each one has exactly one line that Python refuses to accept.
        </p>
        <ol className="max-w-md space-y-3 text-left font-mono text-sm leading-relaxed text-zinc-300">
          <li><span className="text-green-400">01</span>&nbsp; Click the line number that breaks the program.</li>
          <li><span className="text-green-400">02</span>&nbsp; The fault is grammar, never meaning. The logic is fine.</li>
          <li><span className="text-green-400">03</span>&nbsp; <b className="text-red-400">{LIVES} wrong clicks for the whole run.</b></li>
          <li><span className="text-green-400">04</span>&nbsp; A failed run locks your terminal for 2 minutes.</li>
          <li><span className="text-green-400">05</span>&nbsp; 10 minutes for all six. The clock never pauses.</li>
        </ol>
  ...
  return (
    <div className="mx-auto flex h-screen max-w-3xl flex-col gap-4 overflow-hidden px-6 py-6">
      ...
      <p className="shrink-0 font-mono text-sm text-zinc-400">
        {cleared ? (
          <span className="text-green-300">Found it.</span>
        ) : (
          <>
            {bug.brief}
            <span className="text-zinc-600"> Click the line Python rejects.</span>
          </>
        )}
      </p>
      ...
      <div className="shrink-0">
        {cleared && (
          <>
            <div className="rounded-lg border border-green-500/60 bg-zinc-950 px-5 py-4 font-mono text-sm leading-relaxed text-green-50">
              {bug.reason}
            </div>
            <button
              type="button"
